 ...
 * You can use hashLinkDelete to free the links.
 * @param map
 */
...
/**
 * Returns a pointer to the value of the link with the given key and skip traversing as well.
...
void resizeTable(HashMap* map, int capacity)
{
	...
	for (int i = 0; i < map->capacity; i++) {
		link = map->table[i];
		while (link != 0) {
			hashMapPut(tempMap, link->key, link->value);
			link = link->next;
		}
	}
